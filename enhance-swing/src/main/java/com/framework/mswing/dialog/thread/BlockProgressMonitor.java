...
import com.framework.mswing.dialog.JExceptionDialog;
		...
		addJobListener(new AbstractJobListener() {
			...
			public void jobErrorHappened(final Exception ex) {
				SwingUtilities.invokeLater(new Runnable() {
					@Override
					public void run() {
						JExceptionDialog.traceException(parentFrame, "请求数据异常", ex);
						ex.printStackTrace();
					}
				});
			}
		...
		addJobListener(new AbstractJobListener() {
			...
			public void jobErrorHappened(final Exception ex) {
				SwingUtilities.invokeLater(new Runnable() {
					@Override
					public void run() {
						JExceptionDialog.traceException(parentDialog, "请求数据异常", ex);
						ex.printStackTrace();
					}
				});
			}
