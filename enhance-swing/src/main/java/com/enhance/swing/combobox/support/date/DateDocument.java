...
public class DateDocument extends PlainDocument {
	...
	/******************************************************************
	 ** 函数名称：DateDocument 功能描述：设置此文本框显示的默认值、格式限制和当前日期 入口参数： tc : JTextComponent类型,当前操作的文本框 dateFormat : SimpleDateFormat类型,当前操作文本框的格式限制 initDateTime : String类型,当前日期 返回值：无 调用者：类JDateDocument
	 *******************************************************************/
	public DateDocument(JTextComponent tc, SimpleDateFormat dateFormat) throws UnsupportedOperationException {
		// 当前日期构造
		this(tc, dateFormat, getCurrentDate(dateFormat));
	}

	public DateDocument(JTextComponent tc, SimpleDateFormat dateFormat, String initDateTime) throws UnsupportedOperationException {
		// 设置当前日期格式
	...
	/******************************************************************
	 ** 函数名称：public void insertString(int offset, String s, AttributeSet attributeSet) throws BadLocationException 功能描述：重载原方法,限制用户插入格式为日期格式 入口参数：offset: int型,插入位置 s: String型,插入字符串 attributeSet:
	...
	public void insertString(int offset, String s, AttributeSet attributeSet) throws BadLocationException {
		...
		if (s.length() == 1) {
			...
			toTest = toTest.substring(0, newOffset) + s + toTest.substring(newOffset + 1);
	...
	/**********************************************************************************
	 ** 函数名称：public void remove(int offset, int length) throws BadLocationException 功能描述：重载原方法,删除合适位置的字符串 入口参数：offset: int型,插入位置 length: int型,删除长度 返回值：无 调用者：insertString(int, String,AttributeSet)
	 ***********************************************************************************/
	@Override
	...
	/**********************************************************************************
	 ** 函数名称：public boolean isLegalDate(String strDate) 功能描述：判断插入的长度为10的字符串是否合法 入口参数：intY: int型,年的值 intM: int型,月的值 intD: int型,日的值 返回值：boolean型,真,表示是合法的,假,表示不合法 调用者：insertString(int,
	 * String,AttributeSet)
	 ***********************************************************************************/
	private boolean isValidDate(String strDate) {
		...
		if (!isValid) {
			textComponent.setText(toDateString(intY, intM, intD, intH, intMi, intS));
	...
	private String toDateString(int y, int m, int d, int h, int mi, int s) {
		...
		String strDate = "";
		strDate = strY + strPattern.substring(4, 5) + strM + strPattern.substring(7, 8) + strD;
		if (strPattern.length() == 19) {
			strDate += strPattern.substring(10, 11) + rPad0(2, "" + h) + strPattern.substring(13, 14) + rPad0(2, "" + mi) + strPattern.substring(16, 17) + rPad0(2, "" + s);
		}
		return strDate;
	}
