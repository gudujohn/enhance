...
/**
 * 可以设置进度情况和提示信息的进度条 阻塞但是能够取消，注意点击取消后对话框消失 但若用户代码中没有对isCanceled进行判断处理，则执行线程仍然在后台继续
 */
public class BlockProgressMonitor extends AbstractJobMonitor {
	...
	public void setProgress(double value) {
		...
		if (jobDialog != null) {
			/*
			 * jobDialog.getProgressBar().setValue((int) (value * jobDialog.getProgressBar().getMaximum()));
			 */
			if (isPercentPrompt) {
				if (format == null) {
