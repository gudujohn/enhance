...
/**
 * <p>
 * TableLayout is a layout manager that is more powerful than GridBagLayout yet
 * much easier to use.
 * </p>
 *
 * <b>Background</b>
 ...
 *
 * <p>
 * Using TableLayout is a simple two step process. First, create a grid for your
 * container by specifying row and column sizes using either a TableLayout
 ...
 * y2) identfies the bottom right cell. x2 and y2 are optional. If they are not
 ...
 * appropriately as rows and columns are inserted or removed, just as cells are
 * shifted in a spreadsheet.
 * </p>
 *
 * <p>
 * Rows and columns can be made "hidden" or effectively invisible by setting
 * their size to zero. They can be shown again by setting their size back to a
 * non-zero value. This is very useful for toggle form elements without having
 ...
 * <code>PREFERRED</code> that can be used to specify row or column sizes. There
 * is another constant called <code>MINIMUM</code> that does a similar task
 * using components' minimum sizes instead of their preferred sizes.
 * </p>
 *
 * <p>
 * There is no corresponding <code>MAXIMUM</code> constant for several reasons.
 * First, it is mathematically impossible to honor both the minimum and maximum
 * sizes of more than one component when conflicts arise. For example, say
 * components a and b are in the same row. If a's maximum height is less than
 * b's minimum height, then one of these constraints must be violated. Since
 * TableLayout is a complete, general Cartesian layout manager, it would be
 * possible to specify conflicting constraints if a <code>MAXIMUM
...
public class TableLayout implements java.awt.LayoutManager2, java.io.Serializable, TableLayoutConstants {
	...
	private static final long serialVersionUID = 4738543932294201492L;
	 ...
	 * earlier versions of java.awt.Container. Necessary for supporting older JDKs
...
//******************************************************************************
	...
	public List<Object> getInvalidEntry() {
		List<Object> listInvalid = new LinkedList<>();
	 ...
	 * overlap if they cover at least one common cell. This method is useful for
	 * debugging.
	 *
	...
	public List<Object> getOverlappingEntry() {
		List<Object> listOverlapping = new LinkedList<>();
...
//******************************************************************************
	...
	/**
	 * Assigned sizes to preferred and minimum size columns and rows. This reduces
	 ...
	 * of minimum/preferred width or height will get at least its minimum/preferred
